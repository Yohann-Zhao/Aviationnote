...
exports.handler = async (event) => {
  if (event.httpMethod !== "POST") {
    return jsonResponse(405, { error: "不支持该请求方法" });
  }
  ...
  if (!text) {
    return jsonResponse(400, { error: "缺少 `text` 字段" });
  }
  ...
  if (process.env.TTS_API_KEY) {
    try {
      ...
      console.warn("Google TTS 调用失败:", error.message);
      return jsonResponse(502, { error: `Google Cloud TTS 调用失败：${error.message}` });
    }
  }
...
async function synthesizeWithGoogleTts(text, accent, speakingRate) {
  ...
  if (typeof data.audioContent !== "string" || data.audioContent.length === 0) {
    throw new Error("响应中未包含 audioContent");
  }
