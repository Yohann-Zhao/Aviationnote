import { PRACTICE_MODE_COPY } from "./copy-zh.js";
...
const DEFAULT_CUSTOM_LETTER_ARRAY = DEFAULT_CUSTOM_LETTERS.split("");

export const PRACTICE_MODES = [
  {
    id: "mixed",
    label: PRACTICE_MODE_COPY.mixed.label,
    summary: PRACTICE_MODE_COPY.mixed.summary,
    instruction: PRACTICE_MODE_COPY.mixed.instruction
  },
  {
    id: "transport",
    label: PRACTICE_MODE_COPY.transport.label,
    summary: PRACTICE_MODE_COPY.transport.summary,
    instruction: PRACTICE_MODE_COPY.transport.instruction
  },
  {
    id: "generalAviation",
    label: PRACTICE_MODE_COPY.generalAviation.label,
    summary: PRACTICE_MODE_COPY.generalAviation.summary,
    instruction: PRACTICE_MODE_COPY.generalAviation.instruction
  },
  {
    id: "numeric",
    label: PRACTICE_MODE_COPY.numeric.label,
    summary: PRACTICE_MODE_COPY.numeric.summary,
    instruction: PRACTICE_MODE_COPY.numeric.instruction
  },
  {
    id: "letters",
    label: PRACTICE_MODE_COPY.letters.label,
    summary: PRACTICE_MODE_COPY.letters.summary,
    instruction: PRACTICE_MODE_COPY.letters.instruction
  }
];
...
function buildTransportSample() {
  ...
  return finalizeSample({
    ...
    revealAnswer: `${airline.code}${flightNumber}（可输入：${flightNumber} 或 ${airline.code}${flightNumber}）`,
    choicePrefix: airline.code
  });
}
...
function buildGeneralAviationSample(options = {}) {
  ...
  return finalizeSample({
    ...
    revealAnswer: `${aircraftType} ${suffix}（请输入：${suffix}）`
  });
}
...
function mutateAnswerCharacter(character, allowedLetters) {
  ...
  return pick(options.length > 0 ? options : DEFAULT_CUSTOM_LETTER_ARRAY.filter((candidate) => candidate !== character));
}
...
export function buildAnswerChoices(sample, options = {}) {
  ...
  const allowedLetters = normalizeCustomLetterSet(options.customLetters ?? sample.answer).split("");
  ...
  return shuffle([...choices]).map((value) => ({
    value,
    displayValue: sample.choicePrefix ? `${sample.choicePrefix}${value}` : value,
...
export function buildCallsignSample(options = {}) {
  switch (options.mode) {
    case "mixed":
      return Math.random() < 0.5 ? buildTransportSample() : buildGeneralAviationSample(options);
    case "transport":
