...
export async function fetchTtsAudioUrl(text, options = {}) {
  ...
  if (!response.ok) {
    let message = `TTS 请求失败，状态码 ${response.status}。`;

    if (response.status === 404) {
      message = "未找到 TTS 接口。请使用 `npm run dev` 或 `netlify dev` 启动本地环境，以便提供 /api/tts。";
    }
  ...
  if (radioTone) {
    try {
      ...
      console.warn("无线电后处理失败，已回退到原始 TTS 音频。", error);
    }
  }
...
async function renderBlobWithRadioEffect(audioBlob) {
  ...
  if (!OfflineAudioContextCtor) {
    throw new Error("当前浏览器不支持 OfflineAudioContext。");
  }
